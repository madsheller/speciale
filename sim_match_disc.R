...
dgp <- function(N,
  ...
  return(list(obs=obs, y=Y, U_Y=U_Y, cf=c(y0,y1),
...
matchfit <- function(pi,
  ...
  Augm_Response[which(Z != T, arr.ind = T)][prune] <- imp
  ...
  imp <- c(acc, specif, sensiv)
  
  ## Evaluate performance of random guessing
  pred <- rbinom(length(cf), 1, prob = sum(y)/N)
  ...
  rand <- c(acc, specif, sensiv)
  
  perf <- matrix(c(imp, rand), ncol=2)
  colnames(perf) <- c("imp", "random")
